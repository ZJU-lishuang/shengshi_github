...
def convert_labelme_json_format(data_dir, out_dir):
    ...
    for data_set, ann_dir in zip(sets, ann_dirs):
        ...
        for root, _, files in os.walk(ann_dir):
            for filename in files:
                if filename.endswith(ends_in):
                    if len(h5_imgname) % 50 == 0:
                        print("Processed %s images" % (len(h5_imgname)))
                    ...
                    for objects_ann in objects_anns:
                        ...
                        h5_imgnameascii = np.fromstring((filename[:-(5)] + '.jpg'), dtype=np.uint8)
                        
                        #h5_imgname.append(h5_imgnameascii.tolist())
                        
                        h5_imgname.append(image['file_name'].encode('ascii'))
                        #h5_imgname.append((filename[:-(5)] + '.jpg').encode('ascii'))
                        h5_bndbox.append([[x1,y1,x2,y2]])
                        #h5_keypoints.append(reduce(operator.add,objects_ann['keypoints'][2:]))
                        h5_keypoints.append(objects_ann['keypoints'][2:])
                        
                        #annotations.append(ann)
        # ann_dict['images'] = images
        # categories = [{"id": category_dict[name], "name": name} for name in category_dict]
        # categories[0]['supercategory']='person'
        # categories[0]['keypoints']=['head','neck',
        ...
        # categories[0]['skeleton']=[[1,0],[2,1],[3,2],[4,3],[5,2],[6,5],[7,6],[8,1],[9,8],[10,9],[11,5],[11,8],[12,11],[13,12]]
        ...
        h5_keypoints=np.delete(h5_keypoints,2,axis=2)
        
        f=h5py.File("/home/lishuang/Disk/pytorch/mydataset.h5","w")
        f.create_dataset('bndbox',data=h5_bndbox)
        f.create_dataset('imgname',data=h5_imgname)
        f.create_dataset('part',data=h5_keypoints)

        annot = h5py.File("/home/lishuang/Disk/pytorch/mydataset.h5")
        for k in annot.keys():
            print(k)
