...
def candidate_reselect(bboxes, bboxes_scores, pose_preds):
    ...
    group_size = {k: {} for k in range(_channels)}
    ...
    for n, person in pose_preds.items():
        ...
        final_pose = torch.zeros(_channels, 2)
        final_score = torch.zeros(_channels, 1)
