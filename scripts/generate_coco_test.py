...
def convert(xmlFiles, json_file):
...
if __name__ == '__main__':
    ...
    base_dir = "/home/lishuang/Disk/shengshi_data/new_13pointtrainlistcheck" 
    ...
    for dir in subdirs(base_dir):
        FigDirectory = os.path.join(base_dir, dir,'JPEGImages')
        ...
        for file in subfiles(FigDirectory):
            ...
            if os.path.splitext(file_path)[1] == tag:
                ...
                # image = {'file_name': filename, 'height': height, 'width': width,
                #          'id':image_id}
                image = {'file_name': (dir+'/'+'JPEGImages'+'/'+filename+'.jpg'), 'height': height, 'width': width,
                            'id':image_id}
                json_dict['images'].append(image)
    ...
    # convert(fig_names_all,json_dir)
