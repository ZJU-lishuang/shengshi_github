...
"""
def divideTrainValidationTest(source_path,train_path,validation_path,test_path=None):
    """先获取五类图像的名称列表和类别数目"""
    classes_name_list,classes_num=getClassesMes(source_path)
    ...
    for i in range(0,classes_num):
        ...
        train_image_list=source_image_dir[0:int(train_percent*len(source_image_dir))]
        if test_path:
            validation_image_list=source_image_dir[int(train_percent*len(source_image_dir)):int((train_percent+val_percent)*len(source_image_dir))]
            test_image_list=source_image_dir[int((train_percent+val_percent)*len(source_image_dir)):]
        else:
            validation_image_list=source_image_dir[int(train_percent*len(source_image_dir)):]
        """
...
if __name__=='__main__':
    data_path = '/home/lishuang/Disk/shengshi_data/new_anti_tail/classificationtotal/'
    source_path = '/home/lishuang/Disk/shengshi_data/new_anti_tail/classification/'
    train_path = data_path+'train/'
    validation_path =data_path+'validation/'
    test_path = data_path+'test/'
    mkTotalDir(data_path)
    divideTrainValidationTest(source_path, train_path, validation_path)
