...
#     plt.bar(range(len(num_list)), num_list,label="train",color='SkyBlue',align='center')
    plt.barh(range(len(num_list)), num_list,label="train",color='SkyBlue',align='center')
    for b, a in zip(range(len(num_list)), num_list):
#         plt.text(a, b + 0.05, '%.0f' % b, ha='center', va='bottom', fontsize=10)
        plt.text(a , b, '%.0f' % a,ha='left', va='center', fontsize=4)
    plt.yticks(range(len(num_list)),name_list,fontproperties=myfont, fontsize=4)
    plt.ylabel('class')
    plt.xlabel('num')
